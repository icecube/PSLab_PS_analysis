...
class SkyMap:
    ...
    def AddHistogramFile(self, file):
        ...
        if (f.IsOpen()):
            roothist = f.Get("hAllSkyFine")
            roothistfine = f.Get("hAllSkyFine")
            #roothist = ROOT.TH2D(f.Get("hAllSky"))
            #roothistfine = ROOT.TH2D(f.Get("hAllSky"))
        else:
    ...
    def Plot(self, proj, clockwise=False):
        ...
        if(proj == "rectangular"):
            ...
            if clockwise == True:
                ...
                plt.text(-np.pi - xmargin, 0, "0$^{\circ}$", size="large", weight='semibold',**alignment)
                plt.text(np.pi + xmargin, 0, "360$^{\circ}$", size="large", weight='semibold', **alignment)
        ...
        if clockwise == False:
            ...
            plt.scatter((360 - self.ra_maxN - 180.0) * np.pi/180, self.dec_maxN * np.pi/180, lw=2, facecolor='none', marker='o', s=60)
            plt.scatter((360 - self.ra_maxS - 180.0) * np.pi/180, self.dec_maxS * np.pi/180, lw=2, facecolor='none', marker='o', s=60)

        else:
            plt.scatter((self.ra_maxN - 180.0) * np.pi/180, self.dec_maxN * np.pi/180, lw=2, facecolor='none', marker='o', s=60)
